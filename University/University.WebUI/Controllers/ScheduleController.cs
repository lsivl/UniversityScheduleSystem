...
namespace University.WebUI.Controllers
{
    public class ScheduleController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Schedule(int id, string typeData)
        {
            ...
            {
                currentFaculty = repository.DataBase.Faculties.Where(p => p.FacultyID == id).FirstOrDefault();
            ...
            {
                var currentStream = repository.DataBase.Streams.Where(p => p.StreamID == id).FirstOrDefault();
            ...
            {
                var currentGroup = repository.DataBase.Groups.Where(p => p.GroupID == id).FirstOrDefault();
                var currentStream = repository.DataBase.Streams.Where(p => p.StreamID == currentGroup.StreamID).FirstOrDefault();
                //var currentCathedra = repository.DataBase.Cathedras.Where(p=>p.CathedraID==currentStream.CathedraID).FirstOrDefault();
                currentFaculty = repository.DataBase.Faculties.Where(p => p.FacultyID == currentStream.FacultyID).FirstOrDefault();
                //cathedraName = currentCathedra.FullName;
            ...
            {
                var teacher = repository.DataBase.Teachers.Where(p => p.TeacherID == id).FirstOrDefault();
            ...
            {
                var classroom = repository.DataBase.Classrooms.Where(p => p.ClassroomID == id).FirstOrDefault();
            ...
            {
                var cathedra = repository.DataBase.Cathedras.Where(p => p.CathedraID == id).FirstOrDefault();
        ...
        [HttpPost]
        public ActionResult FacultyStreams(int id, string courseValue)
        {
            int yearOfStudy = Convert.ToInt32(courseValue.Remove(1));
         //   int currentFacultyId = repository.DataBase.Faculties.Where(p => p.FacultyID == id).Select(p => p.FacultyID).FirstOrDefault();
            ...
            List<Stream> streams = repository.DataBase.Streams.Where(p => p.FacultyID == id).Where(p => p.YearOfStudy == yearOfStudy).ToList();
        ...
        [HttpPost]
        public ActionResult TeacherClassroom(int id, string TypeData)
        {
        ...
        Selection:
            ...
            else if (currentProperty[3].Trim() == "5")
            {
                ...
                if (busyDB == null)
                {
                    ...
                    //};
                    var unitedGroup = repository.DataBase.UnitedGroups.Where(p => p.GroupID == currentGroupId).Where(p => p.SubjectID == subjectId).FirstOrDefault();
                    List<Group> unitedGroups = new List<Group>();
                    List<Stream> StreamsUnitedGroups = new List<Stream>();
                     if (unitedGroup != null)
                        {
                            var unitedGroupsTable = repository.DataBase.UnitedGroups.Where(p => p.SubjectID == unitedGroup.SubjectID).Where(p => p.Row == unitedGroup.Row).Where(p => p.CathedraID == unitedGroup.CathedraID).ToList();
                            foreach (var item in unitedGroupsTable)
                            {
                                 Group currentUnitedGroup = repository.DataBase.Groups.Where(p => p.GroupID == item.GroupID).FirstOrDefault();
                                 unitedGroups.Add(currentUnitedGroup);
                                 if (StreamsUnitedGroups.Where(p => p.StreamID == currentUnitedGroup.StreamID).Count() == 0)
                                 {
                                     StreamsUnitedGroups.Add(repository.DataBase.Streams.Where(p => p.StreamID == currentUnitedGroup.StreamID).FirstOrDefault());
                                 }
                            }
                        }
                    ...
                    if (lessonTypeId == 1)
                    {
                        ...
                        {
                            if (StreamsUnitedGroups.Select(p=>p.StudentsCount).Sum() > classrrom.Capacity)
                            {
                                lessonChange.TextError = "Помилка!\nКількість студентів об'єднаних груп перевищує місткість аудиторії.";
                                correct = false;
                            }
                        }
                    }
                    if (correct == true)
                    {
                        if (coupled > 1)
                        {
                            for (int i = 0; i < coupled; i++)
                            {
                                if (unitedGroup != null)
                                {
                                    currentTime.LessonTimeID += i;
                                    ScheduleHelper.SaveGroups(repository, unitedGroups, currentTime, lessonTypeId, subjectId, teacherId, classroomId);
                                }
                                else repository.DataBase.Database.ExecuteSqlCommand("exec [dbo].[csp_LessonSave] {0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}", currentGroupId, currentTime.WeekNum, currentTime.DayNumber, currentTime.LessonTimeID + i, lessonTypeId, subjectId, teacherId, classroomId);
                            }
                        }
                        else
                        {
                            if (unitedGroup != null) ScheduleHelper.SaveGroups(repository, unitedGroups, currentTime, lessonTypeId, subjectId, teacherId, classroomId);
        ...
        public ActionResult DeleteLesson(string ParentNode, int CurrentGroupId, string TdElementsID)
        {
            ...
            repository.DataBase.Database.ExecuteSqlCommand("exec [dbo].[csp_SetScheduleUpdated]");
